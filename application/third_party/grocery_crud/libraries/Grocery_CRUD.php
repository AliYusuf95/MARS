...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function exportToExcel($state_info = null)
	{
		...
		@ob_end_clean();
		$this->_export_to_excel_phpExcel($data);
	}
    ...
    protected function _export_to_excel_phpExcel($data)
    {
        ...
        foreach($data->columns as $column){
            $sheet_row[] = $column->display_as;
        }
        $sheet[] = $sheet_row;
        foreach($data->list as $num_row => $row){
            $sheet_row = array();
            foreach($data->columns as $column){
                $sheet_row[] = $this->_trim_export_string($row->{$column->field_name});
            }
            $sheet[] = $sheet_row;
        }
        ...
        foreach($sheet as $row => $columns) {
            foreach($columns as $column => $col_data) {
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($column, $row + 1, $col_data);
                $objPHPExcel->getActiveSheet()->getColumnDimensionByColumn($column)->setAutoSize(true);
            }
        }

        $objPHPExcel->getActiveSheet()
            ->getStyle('A1:'.$objPHPExcel->getActiveSheet()->getHighestColumn().'1')
            ->getFill()
            ->setFillType(PHPExcel_Style_Fill::FILL_SOLID)
            ->getStartColor()
            ->setARGB('e3e3e3');

        $filename = $data->subject."-".date("Y-m-d_H:i:s").".xlsx"; //save our workbook as this file name
	...
	protected function get_dropdown_input($field_info,$value)
	{
		...
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}' class='chosen-select chosen-rtl' data-placeholder='".$select_title."'>";
	...
	protected function get_set_input($field_info,$value)
	{
		...
		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}[]' multiple='multiple' size='8' class='chosen-multiple-select chosen-rtl' data-placeholder='$select_title' style='width:510px;' >";
	...
	protected function get_relation_input($field_info,$value)
	{
		...
		$ajax_or_not_class = $using_ajax ? 'chosen-select chosen-rtl' : 'chosen-select chosen-rtl';
	...
	protected function get_relation_n_n_input($field_info_type, $selected_values)
	{
		...
		{
			$css_class = $has_priority_field || $is_ie_7 ? 'multiselect': 'chosen-multiple-select chosen-rtl';
